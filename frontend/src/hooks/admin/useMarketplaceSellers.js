...
export function useMarketplaceSellers(filters) {
  ...
  const refresh = useCallback(async (background = false) => {
    ...
    catch (caught) { if (caught?.name !== "AbortError") { setError(caught instanceof Error ? caught.message : "Marketplace sellers could not be loaded."); if (background) setStale(true); } }
  ...
  useEffect(() => {
    ...
    return () => { window.clearInterval(interval); document.removeEventListener("visibilitychange", visible); active.current?.abort(); };
