...
import { ScatterPoint } from "@/data/analytics/salesRevenueData";
...
export function ScatterChart({ data = [], height = 170, className = "" }: ScatterChartProps) {
  ...
  const highMarginData = data.filter((d) => d.category === "High Margin");
  const midMarginData = data.filter((d) => d.category === "Mid Margin");
  const lowMarginData = data.filter((d) => d.category === "Low Margin");
  ...
  return (
    <div className={`w-full flex flex-col justify-between ${className}`} style={{ height }}>
      ...
      <div className="flex items-center justify-center flex-wrap gap-1.5 mb-1 text-[9px] font-bold text-slate-700">
        ...
        <span className="inline-flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-red-600 inline-block" /> Low
        </span>
      </div>
      ...
      <div className="w-full flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <RechartsScatterChart margin={{ top: 5, right: 10, left: -15, bottom: 5 }}>
            ...
            />
            <Scatter name="High Margin" data={highMarginData} fill="#16a34a" />
            <Scatter name="Mid Margin" data={midMarginData} fill="#ea580c" />
            <Scatter name="Low Margin" data={lowMarginData} fill="#dc2626" />
