...
import { WORKFLOW_FILTERS, QUICK_FILTERS } from '@/lib/administration/workflows/workflows.constants';
        ...
        <div className="col-span-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search workflows, policies, instances..."
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pt-2 border-t border-gray-150 text-[10px]">
        <div className="flex items-center gap-1.5 flex-wrap min-w-0">
          ...
          {QUICK_FILTERS.map((qf) => {
            ...
            if (qf.label === 'Active') colorClass = 'text-emerald-700';
            if (qf.label === 'Pending Approval' || qf.label === 'SLA Warning' || qf.label === 'Escalated' || qf.label === 'Step-Up Required') colorClass = 'text-amber-700';
            if (qf.label === 'SLA Breach' || qf.label === 'Blocked' || qf.label === 'High Risk' || qf.label === 'Needs Attention') colorClass = 'text-rose-700';
            ...
            return (
              ...
              >
                <span>{qf.label}</span>
                <span className={`text-[8px] font-bold opacity-80 ${isActive ? 'text-white' : ''}`}>{qf.count}</span>
              </button>
            );
          })}
        </div>
