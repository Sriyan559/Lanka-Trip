...
interface SaveDataViewModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (viewName: string) => void;
}

export function SaveDataViewModal({
  isOpen,
  onClose,
  onSave,
}: SaveDataViewModalProps) {
  const [viewName, setViewName] = useState("Pending Import Review Preset");
      ...
      <div className="bg-white rounded-xl shadow-2xl border border-line w-full max-w-md overflow-hidden flex flex-col">
        <div className="p-4 border-b border-line flex items-center justify-between bg-slate-50">
          <div>
            ...
            <h2 className="text-base font-extrabold text-ink">Save Custom Filter View</h2>
          </div>
        ...
        <div className="p-5 space-y-4 text-[12px]">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-[11px] space-y-1 text-slate-600">
            ...
            <div>• Status Tabs: Active Filter Selection</div>
            <div>• Grid Column Ordering & Sort Direction</div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-line bg-slate-50 flex items-center justify-end gap-2">
          ...
          >
            <Bookmark size={13} /> Save Preset View
          </button>
        </div>
      </div>
    </div>
