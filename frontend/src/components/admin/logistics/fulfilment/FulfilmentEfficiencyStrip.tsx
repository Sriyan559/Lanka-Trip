...
export function FulfilmentEfficiencyStrip() {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-1.5 text-[9.5px]">
      {items.map((item, idx) => (
        <div key={idx} className="bg-white p-1.5 sm:p-2 rounded-xl border border-line shadow-xs flex items-center justify-between gap-1">
          <div className="flex items-center gap-1.5 min-w-0">
            <div className="p-1 rounded bg-canvas border border-line flex-shrink-0">
              {item.icon}
            </div>
            <div className="min-w-0">
              <div className="text-[9px] font-semibold text-muted truncate">{item.label}</div>
              <div className="text-xs font-bold text-ink truncate">{item.value}</div>
            </div>
          </div>

          <span className={`text-[8.5px] font-bold px-1 py-0.2 rounded flex items-center gap-0.5 flex-shrink-0 ${
