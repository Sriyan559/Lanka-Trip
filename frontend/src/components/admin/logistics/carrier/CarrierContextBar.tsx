...
interface CarrierContextBarProps {
  lastSynced?: string;
  onRefresh?: () => void;
}

export function CarrierContextBar({
  lastSynced = "May 26, 2025 10:15 AM",
  onRefresh,
}: CarrierContextBarProps) {
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-2.5 shadow-2xs mb-3 space-y-2">
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-1 gap-x-4 text-[11px] text-gray-600 border-b border-gray-100 pb-2">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
          ...
          <div>
            <span className="text-gray-400 font-normal">Carrier Scope: </span>
            <span className="font-semibold text-gray-900">Active Carrier Network</span>
          </div>
          <div>
      ...
      {/* Service Health Indicators Row (8 Badges + Access Context & Policy) */}
      <div className="flex flex-wrap items-center justify-between gap-y-1 gap-x-2 pt-0.5">
        <div className="flex flex-wrap items-center gap-1.5">
          <HealthBadge label="Carrier Registry Health" status="Healthy" />
          <HealthBadge label="Tracking Gateway Health" status="Healthy" />
          <HealthBadge label="Pickup Integration Health" status="Healthy" />
          <HealthBadge label="Delivery Event Health" status="Healthy" />
          <HealthBadge label="POD Integration Health" status="Healthy" />
          <HealthBadge label="COD Integration Health" status="Healthy" />
          <HealthBadge label="Carrier Service Health" status="Healthy" />
