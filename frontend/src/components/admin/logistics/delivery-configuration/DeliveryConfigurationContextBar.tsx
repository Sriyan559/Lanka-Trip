...
interface DeliveryConfigurationContextBarProps {
  lastSynced?: string;
  onRefresh?: () => void;
}

export function DeliveryConfigurationContextBar({
  lastSynced = "May 26, 2025 10:15 AM",
  onRefresh,
}: DeliveryConfigurationContextBarProps) {
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-2.5 shadow-2xs mb-3 space-y-2">
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-1 gap-x-4 text-[11px] text-gray-600 border-b border-gray-100 pb-2">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
          ...
          <div>
            <span className="text-gray-400 font-normal">Delivery Scope: </span>
            <span className="font-semibold text-gray-900">Active Delivery Configuration Network</span>
          </div>
          <div>
      ...
      {/* 8 Health Badges + Access Context & Record Policy Row */}
      <div className="flex flex-wrap items-center justify-between gap-y-1 gap-x-2 pt-0.5">
        <div className="flex flex-wrap items-center gap-1.5">
          <HealthBadge label="Delivery Configuration Health" status="Healthy" />
          <HealthBadge label="Carrier Eligibility Health" status="Healthy" />
          <HealthBadge label="Rate Engine Health" status="Healthy" />
          <HealthBadge label="Capacity Service Health" status="Healthy" />
          <HealthBadge label="SLA Engine Health" status="Healthy" />
          <HealthBadge label="Zone Mapping Health" status="Healthy" />
          <HealthBadge label="Calendar Service Health" status="Healthy" />
          <HealthBadge label="Reconciliation Dependency Health" status="Healthy" />
        </div>
