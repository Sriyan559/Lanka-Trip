...
import { FN14_RIGHT_RAIL, FN14_HEALTH_METRICS } from '@/data/mockReportsImportExportAuditData';
...
const QUEUE_ICON_MAP: Record<string, React.ReactNode> = {
  ...
  Shield: <Shield size={12} className="text-purple-600" />,
};

export function RightReportsSidebar() {
  const [expandedAlerts, setExpandedAlerts] = useState(false);

  return (
    <div className="flex flex-col gap-3">
      {/* ── 1. Finance Data Governance Health Gauge ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <p className="text-[10px] font-bold text-gray-600 uppercase tracking-wide">
            Finance Data Governance Health
          </p>
          <span className="text-[9px] font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200">
            {FN14_RIGHT_RAIL.healthState}
          </span>
        </div>
          ...
          <div className="relative w-20 h-20 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
              ...
              <path
                ...
                strokeDasharray={`${FN14_RIGHT_RAIL.healthScore}, 100`}
                strokeWidth="3.5"
                strokeLinecap="round"
                stroke="currentColor"
                fill="none"
        ...
        <div className="flex flex-col gap-1 text-[10px]">
          {FN14_HEALTH_METRICS.slice(0, 8).map((m) => (
            <div key={m.label} className="flex items-center justify-between gap-1">
              <span className="text-gray-600 truncate">{m.label}</span>
              <span className="font-bold text-emerald-700 font-mono">{m.score}/100</span>
            </div>
          ))}
        </div>
      </div>

      {/* ── 2. Priority Finance Data Alerts ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
            <p className="text-[10px] font-bold text-gray-600 uppercase tracking-wide">
              Priority Finance Data Alerts ({FN14_RIGHT_RAIL.priorityAlerts.length})
            </p>
          </div>
          <button
        ...
        <div className="flex flex-col gap-1.5 text-[10px]">
          {(expandedAlerts
            ? FN14_RIGHT_RAIL.priorityAlerts
            : FN14_RIGHT_RAIL.priorityAlerts.slice(0, 4)
          ).map((alert) => (
            <div
              key={alert.id}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2 text-[10px]">
        ...
        <div className="grid grid-cols-2 gap-2 text-center font-mono">
          <div className="p-1.5 bg-gray-50 border border-gray-100 rounded">
            <span className="text-[8px] text-gray-400 font-semibold block uppercase">Report Summary</span>
            <span className="text-xs font-bold text-gray-900">{FN14_RIGHT_RAIL.reportSummary.count}</span>
          </div>
          <div className="p-1.5 bg-gray-50 border border-gray-100 rounded">
            <span className="text-[8px] text-gray-400 font-semibold block uppercase">Import Summary</span>
            <span className="text-xs font-bold text-gray-900">{FN14_RIGHT_RAIL.importSummary.count}</span>
          </div>
          <div className="p-1.5 bg-gray-50 border border-gray-100 rounded">
            <span className="text-[8px] text-gray-400 font-semibold block uppercase">Export Summary</span>
            <span className="text-xs font-bold text-gray-900">{FN14_RIGHT_RAIL.exportSummary.count}</span>
          </div>
          <div className="p-1.5 bg-gray-50 border border-gray-100 rounded">
            <span className="text-[8px] text-gray-400 font-semibold block uppercase">Audit Summary</span>
            <span className="text-xs font-bold text-gray-900">{FN14_RIGHT_RAIL.auditSummary.count}</span>
          </div>
        </div>

        <div className="divide-y divide-gray-100 pt-1">
          <div className="flex justify-between py-0.5">
            <span className="text-gray-500">Security Summary</span>
            <span className="font-mono font-bold text-emerald-700">{FN14_RIGHT_RAIL.securitySummary.score}</span>
          </div>
          <div className="flex justify-between py-0.5">
            <span className="text-gray-500">Reconciliation Summary</span>
            <span className="font-mono font-bold text-emerald-700">{FN14_RIGHT_RAIL.reconciliationSummary.score}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-1.5 text-[10px]">
          {FN14_RIGHT_RAIL.quickQueues.map((q) => (
            <button
              key={q.label}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-1.5">
        <p className="text-[10px] font-bold text-gray-600 uppercase tracking-wide mb-0.5">
          Final Finance Data Actions
        </p>

        {FN14_RIGHT_RAIL.actionButtons.map((btn) => (
          <button
            key={btn}
