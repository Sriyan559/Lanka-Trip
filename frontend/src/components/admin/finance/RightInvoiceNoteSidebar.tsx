...
import { FN11_RIGHT_RAIL } from '@/data/mockInvoicesNotesData';
...
export function RightInvoiceNoteSidebar() {
  const [expandedAlerts, setExpandedAlerts] = useState(false);

  return (
    <div className="flex flex-col gap-3">
      {/* ── 1. Invoice & Notes Intelligence (Gauge + 7 Health Dimensions) ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
          ...
          <div className="relative w-20 h-20 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
              ...
              <path
                ...
                strokeDasharray={`${FN11_RIGHT_RAIL.healthScore}, 100`}
                strokeWidth="3.5"
                strokeLinecap="round"
                stroke="currentColor"
                fill="none"
        ...
        <div className="flex flex-col gap-1 text-[10px]">
          {FN11_RIGHT_RAIL.healthMetrics.map((m) => (
      ...
      {/* ── 2. Priority Document Alerts (6) ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
            <p className="text-[10px] font-bold text-gray-600 uppercase tracking-wide">
              Priority Document Alerts ({FN11_RIGHT_RAIL.priorityAlerts.length})
            </p>
          </div>
          <button
        ...
        <div className="flex flex-col gap-1.5 text-[10px]">
          {(expandedAlerts
            ? FN11_RIGHT_RAIL.priorityAlerts
            : FN11_RIGHT_RAIL.priorityAlerts.slice(0, 4)
          ).map((alert) => (
            <div
              key={alert.id}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2 text-[11px]">
        ...
        <div className="flex justify-between items-center font-mono">
          <span className="text-gray-500 text-[10px]">Count: {FN11_RIGHT_RAIL.invoiceSummary.count}</span>
          <span className="font-bold text-gray-900">{FN11_RIGHT_RAIL.invoiceSummary.val}</span>
        </div>
        ...
        <div className="flex justify-between items-center font-mono">
          <span className="text-gray-500 text-[10px]">Count: {FN11_RIGHT_RAIL.creditNoteSummary.count}</span>
          <span className="font-bold text-amber-700">{FN11_RIGHT_RAIL.creditNoteSummary.val}</span>
        </div>
        ...
        <div className="flex justify-between items-center font-mono">
          <span className="text-gray-500 text-[10px]">Count: {FN11_RIGHT_RAIL.debitNoteSummary.count}</span>
          <span className="font-bold text-red-600">{FN11_RIGHT_RAIL.debitNoteSummary.val}</span>
        </div>

        {/* Ageing Summary */}
        <div className="flex justify-between items-center border-b border-gray-100 pb-1 pt-1">
          <p className="text-[10px] font-bold text-gray-600 uppercase tracking-wide">
            Ageing &amp; Approval Summary
          </p>
        </div>
        <div className="divide-y divide-gray-100 text-[10px]">
          <div className="flex justify-between py-0.5">
            <span className="text-gray-500">Overdue Value</span>
            <span className="font-bold text-red-600 font-mono">{FN11_RIGHT_RAIL.ageingSummary.overdueVal}</span>
          </div>
          <div className="flex justify-between py-0.5">
            <span className="text-gray-500">Overdue Docs</span>
            <span className="font-bold text-red-600 font-mono">{FN11_RIGHT_RAIL.ageingSummary.overdueDocs}</span>
          </div>
          <div className="flex justify-between py-0.5">
            <span className="text-gray-500">Pending Approval</span>
            <span className="font-bold text-amber-700 font-mono">{FN11_RIGHT_RAIL.approvalSummary.pending}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-1.5 text-[10px]">
          {FN11_RIGHT_RAIL.quickQueues.map((q) => (
            <button
              key={q.label}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-1.5">
        ...
        {FN11_RIGHT_RAIL.actionButtons.map((btn) => (
          <button
            key={btn}
