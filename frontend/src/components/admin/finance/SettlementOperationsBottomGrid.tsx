...
import { FN09_OPERATIONS_CARDS } from '@/data/mockSettlementData';

export function SettlementOperationsBottomGrid() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3 text-xs">
      {FN09_OPERATIONS_CARDS.map((card) => (
        <div
          key={card.num}
            ...
            <div className="flex flex-col gap-1 text-[11px]">
              {card.metrics?.map((m) => (
                <div key={m.label} className="flex justify-between items-center">
                  ...
                  <span className={`font-mono font-bold ${m.isNegative ? 'text-red-600' : 'text-gray-900'}`}>
                    {m.val}
                  </span>
                </div>
              ))}
