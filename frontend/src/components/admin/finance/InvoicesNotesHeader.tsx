...
export function InvoicesNotesHeader() {
  ...
  return (
    <div className="flex flex-col gap-1.5 pb-2 border-b border-gray-200">
      ...
      <nav className="flex items-center gap-1 text-[11px] font-semibold text-gray-500">
        ...
        <span>/</span>
        <span className="text-gray-900 font-bold">Invoices &amp; Notes</span>
      </nav>
      ...
      <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5 max-w-4xl leading-relaxed">
            Govern invoices, credit notes, debit notes, approvals, delivery, payment application, ageing, disputes, reconciliation and immutable audit history across the beauty marketplace.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap shrink-0">
          <button
            onClick={() => toast.success('Exporting Invoice Operations Report...')}
          ...
          >
            <Download size={13} />
            <span>Export Invoice Operations Report</span>
          </button>

          {/* Review Document Exceptions dropdown */}
          <div className="relative">
            ...
            >
              ...
              <span>Review Document Exceptions</span>
              <ChevronDown size={12} />
            </button>
            {exceptionsOpen && (
              <div className="absolute right-0 top-full mt-1 w-64 bg-white border border-gray-200 rounded-lg shadow-xl py-1 z-40 text-xs font-medium text-gray-700">
                {[
                  '624 Overdue Documents',
                  '186 Documents Awaiting Approval',
                  '24 Duplicate Candidates Detected',
                  '18 Reconciliation Exceptions',
                  '9 Document SLA Breaches',
                ].map((item) => (
                  <button
                    key={item}
                    onClick={() => {
          ...
          <button
            onClick={() => toast.error('Opening Review Overdue Documents Queue...')}
          ...
          >
            <FileCheck size={13} />
            <span>Review Overdue Documents</span>
          </button>

          {/* Create Financial Document Dropdown */}
          <div className="relative">
            <button
              onClick={() => setCreateDocOpen((p) => !p)}
            ...
            >
              <Plus size={14} />
              <span>Create Financial Document</span>
              <ChevronDown size={12} />
            </button>
            {createDocOpen && (
              <div className="absolute right-0 top-full mt-1 w-56 bg-white border border-gray-200 rounded-lg shadow-xl py-1 z-50 text-xs font-medium text-gray-700">
                {[
                  ...
                  <button
                    key={docType}
                    onClick={() => {
                      toast.success(`Opening creation modal for ${docType}`);
