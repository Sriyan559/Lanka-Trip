...
export function SettlementSearchFilterBar({ onSearch }: Props) {
  ...
  const handleClearAll = () => {
    ...
    toast('All settlement filters cleared.');
  };
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2.5">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder="Search by settlement reference, beneficiary, batch, bank account..."
        ...
        <div className="flex items-center gap-1.5 shrink-0 flex-wrap">
          ...
          <button
            onClick={() => toast.success('Settlement filter view saved!')}
          ...
          <button
            onClick={() => toast('Refreshing settlement portfolio data...')}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 xl:grid-cols-9 gap-1.5 text-[10px]">
        {(showAllFilters ? SETTLEMENT_FILTERS : SETTLEMENT_FILTERS.slice(0, 18)).map((fName) => (
