...
import { FN05_RIGHT_RAIL } from '@/data/mockRefundData';
...
export function RightRefundSidebar() {
  ...
  return (
    <div className="flex flex-col gap-3">
      {/* ── 1. Refund Operations Health (Gauge + 10 Metrics) ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => toast('Viewing health breakdown...')}
          ...
          <div className="relative w-20 h-20 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
              ...
              <path
                ...
                strokeDasharray={`${FN05_RIGHT_RAIL.healthScore}, 100`}
                strokeWidth="3.5"
                strokeLinecap="round"
                stroke="currentColor"
                fill="none"
        ...
        <div className="flex flex-col gap-1 text-[10px]">
          {FN05_RIGHT_RAIL.healthMetrics.map((m) => (
      ...
      {/* ── 2. Priority Refund Alerts (8) ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
            <p className="text-[10px] font-bold text-gray-600 uppercase tracking-wide">
              Priority Refund Alerts ({FN05_RIGHT_RAIL.priorityAlerts.length})
            </p>
          </div>
          <button
        ...
        <div className="flex flex-col gap-1.5 text-[10px]">
          {(expandedAlerts
            ? FN05_RIGHT_RAIL.priorityAlerts
            : FN05_RIGHT_RAIL.priorityAlerts.slice(0, 4)
          ).map((alert) => (
            <div
              key={alert.id}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => toast('Viewing summary report...')}
            className="text-[10px] text-blue-600 hover:underline font-semibold"
          >
            View
          </button>
        ...
        <div className="divide-y divide-gray-100 text-[11px]">
          {FN05_RIGHT_RAIL.quickSummary.map((item) => (
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        ...
        <div className="grid grid-cols-5 gap-1 text-[10px]">
          {FN05_RIGHT_RAIL.quickQueues.map((q) => (
            <button
              key={q.label}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-1.5">
        ...
        {FN05_RIGHT_RAIL.actionButtons.map((btn) => (
          <button
            key={btn}
