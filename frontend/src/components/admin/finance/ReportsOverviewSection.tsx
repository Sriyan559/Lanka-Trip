...
export function ReportsOverviewSection() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      {/* 1. Reporting & Data-Exchange Trend */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        ...
        <ResponsiveContainer width="100%" height={160}>
          <ComposedChart data={FN14_TREND_DATA} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
            <XAxis dataKey="date" tick={{ fontSize: 8 }} />
            <YAxis tick={{ fontSize: 8 }} />
            <Tooltip />
            <Legend wrapperStyle={{ fontSize: 8 }} />
            <Line type="monotone" dataKey="reportsGen" name="Reports Generated" stroke="#2563eb" strokeWidth={2} dot={{ r: 2 }} />
            <Bar dataKey="reportsDelivered" name="Reports Delivered" fill="#16a34a" opacity={0.8} barSize={6} />
            <Line type="monotone" dataKey="importJobs" name="Import Jobs" stroke="#0ea5e9" strokeWidth={1.5} dot={{ r: 2 }} />
            <Line type="monotone" dataKey="exportsGen" name="Exports Generated" stroke="#f59e0b" strokeWidth={1.5} dot={{ r: 2 }} />
            <Line type="monotone" dataKey="failedJobs" name="Failed Jobs" stroke="#dc2626" strokeWidth={1.5} dot={{ r: 2 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>

      {/* 2. Finance Operation Type Distribution */}
      <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-2">
        <p className="text-[11px] font-bold text-gray-700 uppercase tracking-wide border-b border-gray-100 pb-1.5">
          Finance Operation Type Distribution
        </p>
        <div className="flex items-center gap-2">
          <div className="shrink-0 relative">
            <PieChart width={100} height={100}>
              <Pie
                data={FN14_DONUT_DATA}
                cx={45}
                cy={45}
                innerRadius={28}
                outerRadius={45}
                ...
                dataKey="amount"
              >
                {FN14_DONUT_DATA.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.color} />
                ))}
              </Pie>
            </PieChart>
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
              <span className="text-xs font-extrabold text-gray-900 font-mono leading-tight">1,248</span>
          ...
          <div className="flex flex-col gap-0.5 text-[9px] flex-1 min-w-0">
            {FN14_DONUT_DATA.slice(0, 7).map((seg) => (
              ...
              <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    width: `${Math.min(100, (s.count / 4842) * 100)}%`,
                  }}
                />
              </div>
            </div>
