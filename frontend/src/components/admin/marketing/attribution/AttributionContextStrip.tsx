...
import { AttributionContextData } from "@/data/marketingAttribution.mock";
import { RefreshCw } from "lucide-react";

interface AttributionContextStripProps {
  context: AttributionContextData;
  onRefresh?: () => void;
}

export function AttributionContextStrip({
  context,
  onRefresh,
}: AttributionContextStripProps) {
  return (
    <div className="w-full bg-white border border-gray-200/80 rounded-xl px-3 py-2 text-[11px] font-sans shadow-2xs">
      <div className="flex flex-wrap items-center justify-between gap-y-1.5 gap-x-4">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 divide-x divide-gray-200 text-gray-600">
          ...
          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Analysis Scope:</span>
            <span className="font-semibold text-gray-900">{context.analysisScope || "All Marketing"}</span>
          </div>

          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Attribution Model:</span>
            <span className="font-bold text-[#800020]">{context.attributionModel || "Data-Driven Attribution"}</span>
          </div>

          {/* Finance Revenue Sync */}
          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Finance Revenue Sync:</span>
            <span className="flex items-center gap-1 font-semibold text-emerald-700">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
              {context.financeRevenueSync || "Healthy"}
            </span>
          </div>

          {/* Identity Resolution */}
          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Identity Resolution:</span>
            <span className="flex items-center gap-1 font-semibold text-emerald-700">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
              {context.identityResolution || "Healthy"}
            </span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-gray-600">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-bold text-gray-900">{context.completenessPercent ?? 96}%</span>
            <div className="w-10 h-1.5 bg-gray-200 rounded-full overflow-hidden">
              <div
                className="h-full bg-emerald-500 rounded-full"
                style={{ width: `${context.completenessPercent ?? 96}%` }}
              />
            </div>
          </div>
