...
import { ContentContextData, ContentKpiItem } from "@/data/marketingContent.mock";
import { FileText, CheckCircle2, Clock, AlertTriangle, Layers, Film, Calendar, ShieldCheck, RefreshCw } from "lucide-react";

export function ContentContextStrip({
  context,
  onRefresh,
}: {
  context: ContentContextData;
  onRefresh?: () => void;
}) {
  return (
    <div className="bg-white border border-gray-200/80 rounded-xl px-2.5 py-1.5 shadow-2xs font-sans text-[10px] flex flex-nowrap items-center justify-between gap-x-2.5 overflow-x-auto no-scrollbar">
      <div className="flex items-center gap-2.5 flex-nowrap shrink-0">
        ...
        <div>
          <span className="text-gray-400 font-bold text-[8.5px] block uppercase tracking-tight">CONTENT SCOPE</span>
        ...
        <div>
          <span className="text-gray-400 font-bold text-[8.5px] block uppercase tracking-tight">CATALOGUE MEDIA SOURCE</span>
          <span className="font-bold text-emerald-700 text-[10px] flex items-center gap-1 whitespace-nowrap">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0" />
            {context.catalogueMediaSource}
          </span>
        </div>
        ...
        <div>
          <span className="text-gray-400 font-bold text-[8.5px] block uppercase tracking-tight">BRAND RULES</span>
          <span className="font-bold text-emerald-700 text-[10px] flex items-center gap-1 whitespace-nowrap">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0" />
            {context.brandRules}
          </span>
        ...
        <div>
          <span className="text-gray-400 font-bold text-[8.5px] block uppercase tracking-tight">RIGHTS VALIDATION</span>
          <span className="font-bold text-emerald-700 text-[10px] flex items-center gap-1 whitespace-nowrap">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0" />
            {context.rightsValidation}
          </span>
        </div>
...
export function ContentKpiStrip({
  kpis = [],
}: {
  kpis: ContentKpiItem[];
}) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 font-sans">
      {kpis.map((kpi) => {
        const IconComp = KPI_ICONS[kpi.id] || FileText;
