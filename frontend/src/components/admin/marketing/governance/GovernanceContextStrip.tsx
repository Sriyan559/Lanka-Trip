...
import { GovernanceContextData } from "@/data/marketingGovernance.mock";
import { RefreshCw } from "lucide-react";

interface GovernanceContextStripProps {
  context: GovernanceContextData;
  onRefresh?: () => void;
}

export function GovernanceContextStrip({
  context,
  onRefresh,
}: GovernanceContextStripProps) {
  return (
    <div className="w-full bg-white border border-gray-200/80 rounded-xl px-3 py-2 text-[11px] font-sans shadow-2xs">
      <div className="flex flex-wrap items-center justify-between gap-y-1.5 gap-x-4">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 divide-x divide-gray-200 text-gray-600">
          ...
          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Governance Scope:</span>
            <span className="font-semibold text-gray-900">{context.governanceScope || "All Marketing"}</span>
          </div>

          {/* Consent Source - visually indicates Customer Domain */}
          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Consent Source:</span>
            <span className="flex items-center gap-1 font-semibold text-gray-900">
              {context.consentSource || "Customer Domain"}
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block ml-0.5" />
            </span>
          </div>

          {/* Compliance Source - visually indicates regulatory domain */}
          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Compliance Source:</span>
            <span className="flex items-center gap-1 font-semibold text-emerald-700">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
              {context.complianceSource || "Connected"}
            </span>
          </div>

          {/* Content Rights Sync */}
          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Content Rights Sync:</span>
            <span className="flex items-center gap-1 font-semibold text-emerald-700">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
              {context.contentRightsSync || "Healthy"}
            </span>
          </div>

          <div className="pl-3 flex items-center gap-1">
            <span className="text-gray-400 font-medium">Policy Version:</span>
            <span className="font-bold text-[#800020]">{context.policyVersion || "MKT-GOV v6"}</span>
          </div>
