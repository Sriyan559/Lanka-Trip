...
export function PlacementHeader({ onCreateCampaign }: PlacementHeaderProps) {
    ...
    <div className="flex flex-col gap-2 bg-white p-3 sm:p-4 rounded-xl border border-gray-200/80 shadow-2xs">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-xs font-medium text-gray-500">
          ...
          <span>/</span>
          <span className="text-gray-900 font-semibold">Web & App Campaigns</span>
        </div>

        {/* Header Action Buttons */}
        <div className="flex flex-wrap items-center gap-2">
          {/* 1. Export Placement Report */}
          <Link
          ...
          <div className="relative">
            ...
            {bulkOpen && (
              <div className="absolute right-0 mt-1 w-52 bg-white border border-gray-200 rounded-lg shadow-lg z-30 py-1 text-xs text-gray-700">
                <button
                  onClick={() => setBulkOpen(false)}
                  className="w-full text-left px-3 py-2 hover:bg-gray-50 font-medium"
                >
                  Pause Selected Placements
                </button>
                <button
                  onClick={() => setBulkOpen(false)}
                  className="w-full text-left px-3 py-2 hover:bg-gray-50 font-medium"
                >
                  Re-assign Content Batch
                </button>
                <button
                  onClick={() => setBulkOpen(false)}
                  className="w-full text-left px-3 py-2 hover:bg-gray-50 font-medium"
                >
                  Validate Conflict Priority
                </button>
                <button
                  onClick={() => setBulkOpen(false)}
          ...
          {/* 5. Create Placement Campaign (Crimson Primary Button) */}
          <button
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-gray-900">
          Website, App & Merchandising Campaigns
        </h1>
        <p className="text-xs sm:text-sm text-gray-500 mt-0.5">
          Manage on-site and in-app merchandising placements, targeted experiences, campaign landing content and storefront execution across the ecosystem.
        </p>
      </div>
    </div>
  );
