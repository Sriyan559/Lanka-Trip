...
const rowActions = ["View Listing","Review Listing","Edit Listing","View Product Master","View Seller","Review Compliance","Review Pricing","View Inventory","Publish","Suspend","Reject","View Audit History"];
...
export function MarketplaceListingsView() {
  ...
  const doAction=(action:string,row?:ListingRow)=>{setMenu(null);if(row&&["View Listing","Review Listing","Review Compliance"].includes(action)){router.push(`/admin/marketplace/listings/${encodeURIComponent(row.id)}`);return;}if(["Publish","Suspend","Reject"].includes(action)){setModal(`${action}${row?` ${row.name}`:" selected listings"}`);return;}setModal(action);};
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.layout}><div className={styles.workspace}>
      ...
      <section className={styles.listCard}>
        ...
        <div className={styles.tableWrap}><table><thead><tr><th><input aria-label="Select all listings" type="checkbox" checked={rows.length>0&&selected.length===rows.length} onChange={toggleAll}/></th>{["Listing","Listing ID","Seller","Product","Brand","Business Unit","Channel","Selling Price","Stock","Sales (30D)","Conversion (30D)","Verification","Policy Status","Risk","Listing Status","Updated","Reviewer","Action"].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{rows.length?rows.map((row,i)=><tr key={row.id}><td><input aria-label={`Select ${row.name}`} type="checkbox" checked={selected.includes(row.id)} onChange={()=>setSelected(s=>s.includes(row.id)?s.filter(id=>id!==row.id):[...s,row.id])}/></td><td><div className={`${styles.thumb} ${styles[`thumb${i+1}`]}`}/><Link className={styles.detailLink} href={`/admin/marketplace/listings/${encodeURIComponent(row.id)}`}>{row.name}</Link><small>{row.descriptor}</small></td><td><Link className={styles.idLink} href={`/admin/marketplace/listings/${encodeURIComponent(row.id)}`}>{row.id}</Link></td><td>{row.seller}</td><td>{row.product}</td><td>{row.brand}</td><td>{row.businessUnit}</td><td>{row.channel}</td><td>{row.price}</td><td className={row.stock<50?styles.stockWarning:""}>{row.stock}</td><td>{row.sales}</td><td>{row.conversion}</td><td><span className={`${styles.badge} ${tone(row.verification)}`}>{row.verification}</span></td><td><span className={`${styles.badge} ${tone(row.policy)}`}>{row.policy}</span></td><td><span className={`${styles.badge} ${tone(row.risk)}`}>{row.risk}</span></td><td><span className={`${styles.badge} ${tone(row.status)}`}>{row.status}</span></td><td>{row.updated}</td><td>{row.reviewer}</td><td className={styles.actionCell}><button aria-label={`Actions for ${row.name}`} onClick={()=>setMenu(menu===row.id?null:row.id)}><MoreHorizontal size={16}/></button>{menu===row.id&&<div className={styles.rowMenu}>{rowActions.map(action=><button key={action} onClick={()=>doAction(action,row)}>{action}</button>)}</div>}</td></tr>):<tr><td className={styles.empty} colSpan={19}>No listings match the active filters.</td></tr>}</tbody></table></div>
