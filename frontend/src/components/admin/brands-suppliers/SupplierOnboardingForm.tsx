...
export function SupplierOnboardingForm({ supplierId }: { supplierId?: string }) {
  ...
  useEffect(() => {
    if (supplierId) {
      setLoading(true);
      brandsSuppliersApi.getSupplierDetail(supplierId)
        .then((res: any) => {
          if (res?.supplier) {
            setFormData({
              company_name: res.supplier.company_name || res.supplier.store_name || '',
              legal_name: res.supplier.legal_name || '',
              email: res.supplier.email || '',
              phone: res.supplier.phone || '',
              business_type: res.supplier.business_type || 'distributor',
              country: res.supplier.country || 'Sri Lanka',
            });
          }
        })
        .catch(err => console.error("Failed to fetch supplier for edit:", err))
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (supplierId) {
        await brandsSuppliersApi.updateSupplier(supplierId, formData);
        alert('Supplier draft updated successfully in database!');
      } else {
        await brandsSuppliersApi.createSupplier(formData);
